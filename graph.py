...
class Graph:
  ...
  def explore(self):
    ...
    while current_room != 'treasure room':
      ...
      valid_choices = [node[0] for node in node.get_edges()]
      print('\nYou have accumulated: {0} cost'.format(path_total))
      choice = input('\nWhich room do you move to?')
      if choice not in valid_choices:
        print('please select from these lettes: "{0}"'.format('", "'.join(valid_choices)))
      else:
        ...
        print('\n*** You have chosen: {0} ***\n'.format(current_room))
    print('Made it to the trasure room with {0} cost'.format(path_total))
